...
import {
  ...
  MessageCircle, CheckCircle, Star, Zap, Package, Tag, Clock, Flame,
} from 'lucide-react';
...
import { SITE_NAME, SITE_URL, CATEGORIES_NAV, WHATSAPP_NUMBER } from '../utils/constants';
...
const DEFAULT_SLIDES: Banner[] = [
  { _id: 'default-1', title: 'Votre partenaire tech', subtitle: "en Afrique de l'Ouest", description: 'Laptops, smartphones, TV, électroménager — tout le matériel tech dont vous avez besoin', cta: 'Explorer la boutique', link: '/boutique', image: 'https://images.unsplash.com/photo-1496181133206-80ce9b88a853?w=1200', badge: 'Livraison rapide · Dakar & régions' },
  { _id: 'default-2', title: 'Smartphones dernière génération', subtitle: 'Samsung, iPhone, Xiaomi', description: 'Les meilleurs smartphones au meilleur prix, avec garantie et service après-vente', cta: 'Voir les téléphones', link: '/boutique/telephones', image: 'https://images.unsplash.com/photo-1610945415295-d9bbf067e59c?w=1200', badge: 'Nouveautés 2025' },
  { _id: 'default-3', title: 'Services Réparation & Installation', subtitle: 'Techniciens certifiés', description: 'Réparation PC, téléphones, installation caméras, réseau Wi-Fi', cta: 'Voir nos services', link: '/services', image: 'https://images.unsplash.com/photo-1588702547919-26089e690ecc?w=1200', badge: 'Devis gratuit via WhatsApp' },
];

const PROMO_BANNERS = [
  { slug: 'informatique', label: 'Informatique', title: 'Laptops & PC', sub: 'Dès 150 000 FCFA', image: 'https://images.unsplash.com/photo-1517694712202-14dd9538aa97?w=800', gradient: 'linear-gradient(to right, rgba(0,61,28,0.85) 0%, rgba(0,61,28,0.4) 60%, transparent 100%)', accent: '#FCD116' },
  { slug: 'telephones', label: 'Téléphonie', title: 'Smartphones 2025', sub: 'Samsung · iPhone · Xiaomi', image: 'https://images.unsplash.com/photo-1511707171634-5f897ff02aa9?w=800', gradient: 'linear-gradient(to right, rgba(80,0,100,0.85) 0%, rgba(80,0,100,0.4) 60%, transparent 100%)', accent: '#FCD116' },
];
...
function SectionHeader({ label, labelIcon, title, to, toLabel = 'Voir plus', accent = '#009A44' }: {
  ...
  return (
    <div className="flex items-center justify-between mb-4">
      ...
      <Link to={to} className="text-sm font-medium flex items-center gap-1 no-underline shrink-0" style={{ color: accent }}>
...
export default function HomePage() {
  ...
  const [banners, setBanners] = useState<Banner[]>(DEFAULT_SLIDES);
    ...
    const fetchData = async () => {
      try {
        const [f, n, b, s, bann, promo] = await Promise.all([
          api.get('/products/featured'), api.get('/products/new-arrivals'),
          api.get('/products/best-sellers'), api.get('/services'),
          api.get('/banners'), api.get('/banners/promo'),
        ]);
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="Boutique en ligne de matériel informatique, smartphones, TV et électroménager. Services réparation et installation. Livraison en Afrique de l'Ouest. Paiement Orange Money, Wave." />
        <meta name="keywords" content="informatique Dakar, smartphone Sénégal, laptop prix FCFA, TV écran, réparation téléphone, Wave Orange Money" />
        <link rel="canonical" href={SITE_URL} />

        {/* Open Graph */}
        <meta property="og:type"        content="website" />
        <meta property="og:site_name"   content="Sunu Shop" />
        <meta property="og:title"       content={`${SITE_NAME} — Informatique, Électronique & Tech`} />
        <meta property="og:description" content="Boutique en ligne tech en Afrique de l'Ouest. Laptops, smartphones, TV, électroménager. Livraison Dakar & régions." />
        ...
        <meta name="twitter:title"       content={`${SITE_NAME} — Tech Afrique de l'Ouest`} />
        <meta name="twitter:description" content="Laptops, smartphones, TV, électroménager. Livraison Dakar & régions. Wave, Orange Money." />
        <meta name="twitter:image"       content={`${SITE_URL}/og-home.jpg`} />
      </Helmet>

      {/* 1. Hero + Sidebar */}
      <HeroSlider banners={banners} currentSlide={currentSlide} onSlideChange={setCurrentSlide} />
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 divide-x divide-gray-100">
            {[
              ...
              <div key={title} className="flex items-center gap-3 px-4 py-3">
                <div className="rounded-lg flex items-center justify-center shrink-0" style={{ width: 38, height: 38, backgroundColor: color + '18' }}>
      ...
      <section className="bg-white border-b">
        <div className="container-custom py-3">
          <div className="flex items-center gap-2 overflow-x-auto pb-1" style={{ msOverflowStyle: 'none', scrollbarWidth: 'none' }}>
            {CATEGORIES_NAV.map((cat) => (
              <motion.div key={cat.slug} whileHover={{ y: -2 }} whileTap={{ scale: 0.97 }}>
                <Link to={`/boutique/${cat.slug}`}
                  ...
                  <span className="text-xs font-medium text-gray-700 text-center leading-tight group-hover:text-[#009A44] transition-colors" style={{ whiteSpace: 'nowrap' }}>
                    {cat.name.split(' ')[0]}
                  </span>
                </Link>
              </motion.div>
            ...
            <motion.div whileHover={{ y: -2 }} whileTap={{ scale: 0.97 }}>
              <Link to="/boutique?onSale=true"
                ...
                <span className="text-xs font-semibold text-center leading-tight" style={{ color: '#E31B23', whiteSpace: 'nowrap' }}>Promos</span>
              </Link>
            </motion.div>
          </div>
        </div>
          ...
          <div className="container-custom">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-white font-mono text-xs font-bold" style={{ backgroundColor: '#E31B23' }}>
                  <Clock size={11} className="mr-0.5" />
                  {pad(countdown.h)}:{pad(countdown.m)}:{pad(countdown.s)}
                </div>
              </div>
              <Link to="/boutique?onSale=true" className="text-sm font-medium flex items-center gap-1 no-underline shrink-0" style={{ color: '#E31B23' }}>
                Voir plus <ArrowRight size={13} />
              </Link>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
              {featured.slice(0, 5).map((product) => <ProductCard key={product._id} product={product} />)}
            </div>
          </div>
        </section>
      )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {PROMO_BANNERS.map((b) => (
              <motion.div key={b.slug} whileHover={{ scale: 1.01 }} transition={{ duration: 0.2 }}>
                <Link to={`/boutique/${b.slug}`} className="relative block rounded-xl overflow-hidden no-underline shadow-md" style={{ height: '150px' }}>
                  ...
                  <div className="absolute inset-0 flex flex-col justify-center p-5">
                    <span className="text-xs font-bold uppercase mb-1 px-2 py-0.5 rounded self-start"
                      style={{ backgroundColor: b.accent + '30', color: b.accent, letterSpacing: '0.07em' }}>{b.label}</span>
      ...
      {newArrivals.length > 0 && (
        <section className="bg-white border-t py-4">
          <div className="container-custom">
            <SectionHeader label="Derniers arrivages" labelIcon={<Package size={10} />} title="Nouveautés" to="/boutique?sort=newest" />
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
              {newArrivals.slice(0, 5).map((product) => <ProductCard key={product._id} product={product} />)}
            </div>
          </div>
        </section>
      )}
      ...
      {bestSellers.length > 0 && (
        <section className="bg-white border-t py-4">
          <div className="container-custom">
            <SectionHeader label="Populaires" title="Meilleures Ventes" to="/boutique?sort=bestseller" accent="#009A44" />
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
              {bestSellers.slice(0, 6).map((product) => <ProductCard key={product._id} product={product} />)}
            </div>
          </div>
        </section>
      )}
      ...
      <section className="bg-gray-50 border-t py-10">
        <div className="container-custom">
            ...
            {STATIC_REVIEWS.map((review, i) => (
              <motion.div key={i}
                ...
                <div className="flex items-center gap-3 pt-3 border-t border-gray-100">
                  ...
                  <div>
                    <p className="font-semibold text-sm text-gray-900 mb-0">{review.name} <span>{review.flag}</span></p>
          ...
          <div className="mt-8 text-center">
            <a href={`https://wa.me/${WHATSAPP_NUMBER.replace('+', '')}?text=Bonjour Sunu Shop, j'ai une question.`}
              target="_blank" rel="noopener noreferrer"
              className="inline-flex items-center gap-2 px-6 py-3 rounded-xl font-semibold text-white no-underline transition-opacity hover:opacity-90"
